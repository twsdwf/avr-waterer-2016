...
//  	servoUp();
//  	park();
	Serial1.println("parking...");
  	parkY();
	Serial1.println("Y done. X...");
  	parkX();
	Serial1.println("X done.");
}
...
// 		Serial1.println("should move Y");
	...
	while (xs || ys) {
		if (xs == WDST_PARK) {
			if (/*HIGH == digitalRead(X_BEGIN_PIN)*/analogRead(X_BEGIN_PIN-A0)>= X_BEGIN_STOPVAL) {
