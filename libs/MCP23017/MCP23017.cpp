...
// 	Serial1.print("start read reg of dev ");
// 	Serial1.println(i2caddr, DEC);
// 	Serial1.flush();
	Wire.beginTransmission(MCP23017_ADDRESS | i2caddr);
	wiresend(addr);
	Wire.endTransmission();
// 	Serial1.print("req data from chip");
// 	Serial1.println(i2caddr, DEC);
// 	Serial1.flush();

	Wire.requestFrom(MCP23017_ADDRESS | i2caddr, 1);
// 	Serial1.print("read reg of dev ");
// 	Serial1.println(i2caddr, DEC);
// 	Serial1.flush();
	uint8_t ret = wirerecv();
// 	Serial1.print("got from dev ");
...
/*
void getLastInterruptData(uint8_t& pin, uint8_t& val)
{
	intf = readRegister(MCP23017_INTFA);
	pin = MCP23017_INT_ERR;
	for(int i=0;i<8;i++) if (bitRead(intf, i)) {pin = i; break;}

	// try port B
	intf = readRegister(MCP23017_INTFB);
	for (int i=0;i<8;i++) if (bitRead(intf,i)) {pin = i + 8;break;}
	uint16_t new_state = readGPIOAB();
	val = (new_state & (1<<pin)?1:0);
}*/
